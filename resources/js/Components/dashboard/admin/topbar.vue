...
<template>
	<header class="shrink-0 z-20 flex h-[70px] items-center justify-between border-b border-[#e2e8f0] bg-white px-4 sm:px-8 font-poppins">
		...
		<button
			...
			class="flex h-9 w-9 items-center justify-center rounded-lg text-[#23453e] transition-colors hover:bg-[#eef4f2] focus:outline-none"
			@click="$emit('toggle')"
		>
		...
		<div class="relative" @click.stop>
			<button
				type="button"
				class="flex items-center gap-2 sm:gap-2.5 rounded-full md:rounded-lg py-1 px-1.5 md:px-2.5 md:py-1.5 text-[#23453e] transition-colors hover:bg-[#eef4f2] focus:outline-none"
			...
			>
				...
				<div class="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border border-[#d5e2de] bg-[#eef4f2] shadow-2xs">
					<img
				...
				<span class="hidden md:inline-block font-poppins text-sm font-semibold text-[#23453e] max-w-[200px] lg:max-w-[320px] truncate" :title="userName">
					{{ userName }}
				</span>

				<!-- Dropdown Chevron Arrow (Always visible on all screen sizes) -->
				<svg
					:class="['h-4 w-4 text-[#7b9c94] transition-transform duration-200 shrink-0', showingAccountMenu ? 'rotate-180 text-[#23453e]' : '']"
					fill="none"
					stroke="currentColor"
					stroke-width="2"
					viewBox="0 0 24 24"
			...
			<div
				...
				class="absolute right-0 top-[52px] z-40 min-w-[190px] max-w-[calc(100vw-2rem)] w-max rounded-xl border border-[#d5e2de] bg-white p-1.5 shadow-xl font-poppins space-y-0.5"
			>
				<!-- User Info Header (Mobile Only) -->
				<div class="px-3 py-2 border-b border-[#eef4f2] md:hidden">
					<p class="font-poppins text-xs font-semibold text-[#23453e] whitespace-nowrap truncate" :title="userName">
						{{ userName }}
					</p>
					<p class="font-poppins text-[11px] text-[#6e8d85] mt-0.5">
						Administrator
					</p>
				</div>
